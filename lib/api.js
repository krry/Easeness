...
export async function getPageBySlug(slug, preview) {
  const data = await getClient(preview).fetch(`*[_type == 'page' &&
    slug.current == '${slug}']{
    ${pageFields}
    content
  }`, {slug})
...
export async function getAllDocsWithSlug(docType) {
  const data = await client.fetch(`*[_type == '${docType}']{ 'slug': slug.current }`)
  return data
}
